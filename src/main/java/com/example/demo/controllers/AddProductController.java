...
//        product.getParts().addAll(assparts);
    ...
    public String buyProduct(@RequestParam("productID") int theId, Model theModel) {
        // initialize the productService bean through spring context
        ProductService productService = context.getBean(ProductServiceImpl.class);
        // creating product object called product2 (from product class)
        Product product2 = productService.findById(theId);
        // creating variable to store value of inventory
        int inv = product2.getInv();

        //checking value of inv == 0
        if (inv == 0) {
            ...
            // reduce inventory value by one
            int newInv = inv - 1;

            // set new value of product inventory
            product2.setInv(newInv);
